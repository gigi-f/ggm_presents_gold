...
function rngRangeInt(rng, min, maxInclusive) { return Math.floor(rng() * (maxInclusive - min + 1)) + min; }
...
export function createShopBuilding(scene, doorGridX, doorGridY, opts = {}) {
  ...
  const signTextColor = opts.signTextColor ?? style.signTextColor;
  ...
  // Keep building away from outer walls by at least 1 interior cell
  // Ensure the bottom row of the building always aligns with the door grid Y
  ...
  const maxGY = gridH - heightTiles - 2; // 1-cell gap at bottom
    ...
    const maxHeightFromDoor = Math.max(2, doorGridY - minGY + 1); // tallest allowed without violating top margin
    const minHeightFromDoor = Math.max(2, doorGridY - maxGY + 1); // shortest allowed without violating bottom margin
  ...
  if (topGY < minGY) topGY = minGY; // already reduced height above if needed
  if (topGY > maxGY) topGY = maxGY; // safe clamp (door should be placed to avoid this)
  ...
  // Vertical layout zones (top to bottom): roof -> facade band -> sign band -> door band
  const roofBaseY = -bodyH / 2; // top of body
  // Without roof, start facade band a bit below the top edge for spacing
  ...
  if (ENABLE_ROOF) {
    ...
    const roofH = Math.max(cs, Math.floor(cs));
    // With roof, shift facade zone below roof band
    ...
    if (roofStyle === 'flat') {
      ...
      // Optional shingles (graphics stripes)
      ...
      for (let i = 1; i < shingles; i++) {
        ...
        g.beginPath();
        g.moveTo(- (bodyW + roofOverhang) / 2 + 2, y);
        g.lineTo((bodyW + roofOverhang) / 2 - 2, y);
        g.strokePath();
      }
      building.add(g);
    } else {
      // Gabled: draw a triangular roof with overhang using graphics
      const g = scene.add.graphics();
      g.fillStyle(roofColor, 1);
      // Base sits on the top edge of the body; apex rises at least one cell above base
      const baseY = roofBaseY + trimStroke;
      const topY = baseY - cs; // ensure at least one grid cell tall
  ...
  if (ENABLE_ROOF && addChimney) {
    ...
    const cy = (roofStyle === 'flat')
      ...
      : (baseY - cs * 0.5 + chimH / 2); // sit roughly mid-slope on gabled roof
  ...
  // Place windows strictly between facadeTopY and (signBandY - margin)
  const winMargin = cs * 0.3;
  let rowY = facadeTopY + cs * 0.6;
  for (let r = 0; r < windowRows; r++) {
    if (rowY + windowH / 2 > signBandY - winMargin) break; // don't overlap sign
  ...
  // Side walls (from top to bottom-1)
  for (let gy = topGY + 1; gy < topGY + heightTiles; gy++) {
    addWall(leftGX, gy, cs, cs);
    addWall(leftGX + widthTiles - 1, gy, cs, cs);
  }

  // Bottom wall row with door gap at doorGridX; align gap with door band
  // Create wider clearance around the doorway to ensure easy access
  for (let gx = leftGX; gx < leftGX + widthTiles; gx++) {
    const isDoorRow = (topGY + heightTiles - 1) === doorGridY;
    if (gx === doorGridX && isDoorRow) continue; // leave door gap
    if (isDoorRow && (gx === doorGridX - 1 || gx === doorGridX + 1)) {
      const clearance = 6; // pixels of side clearance into the doorway (increased from 3)
      const w = Math.max(4, cs - clearance * 2);
      const rect = addWall(gx, topGY + heightTiles - 1, w, cs);
      if (gx === doorGridX - 1) {
        // Left jamb: place so its right edge is clearance px from the door cell boundary
        rect.x = gx * cs + (cs - clearance) - (w / 2);
      } else {
        // Right jamb: place so its left edge is clearance px from the door cell boundary
        rect.x = gx * cs + clearance + (w / 2);
      }
      continue;
    }
  ...
  for (let gx = leftGX; gx < leftGX + widthTiles; gx++) {
    for (let gy = topGY; gy < topGY + heightTiles; gy++) {
      if (gx === doorGridX && gy === doorGridY) continue; // door gap
      const key = `${gx},${gy}`;
